...
const RecruitersTab = ({ data, viewMode = 'both', percentageBase = 'users' }) => {
  ...
  const recruiterData = data.daily_by_recruiter || [];

  // Aggregate by recruiter
  const aggregatedData = {};
  recruiterData.forEach(row => {
    const key = row.recruiter || '(Not Assigned)';
    if (!aggregatedData[key]) {
      aggregatedData[key] = { recruiter: key };
  ...
  return (
    <div className="recruiters-tab">
      ...
      <div className="table-controls">
        <span style={{ fontWeight: 600 }}>All Recruiters</span>
      ...
      {/* Selected Recruiter Funnel */}
      {selectedRecruiter && (
        <div className="selected-item-funnel">
          <ConversionFunnel data={selectedRecruiter} title={`Funnel for: ${selectedRecruiter.recruiter?.split('@')[0] || 'Selected Recruiter'}`} />
        </div>
      )}
    </div>
  );
